...
namespace App\Services\Payments {
    ...
    use App\Services\Payments\Asaas\AsaasCustomer;
    use App\Services\Payments\Asaas\AsaasPix;
    use App\Services\Payments\Asaas\AsaasCard;
    use App\Services\Payments\Asaas\AsaasWebhook;
    ...
    class AsaasPaymentProvider implements PaymentProviderInterface
    {
        ...
        private AsaasCustomer $customer;
        private AsaasPix $pix;
        private AsaasCard $card;
        private AsaasWebhook $webhook;
        ...
        public function __construct(PDO $pdo, SubscriptionService $subscriptions)
        {
            ...
            $apiUrl = rtrim($this->env('ASAAS_API_URL', 'https://api-sandbox.asaas.com/v3'), '/');
            $apiKey = $this->env('ASAAS_API_KEY', '');
            $webhookToken = $this->env('ASAAS_WEBHOOK_TOKEN', '');
            $sandboxCpfCnpj = $this->env('ASAAS_SANDBOX_CPF_CNPJ', '');

            $this->client = new AsaasClient($apiUrl, $apiKey);
            $this->customer = new AsaasCustomer($this->client, $sandboxCpfCnpj);
            $this->pix = new AsaasPix($this->client);
            $this->card = new AsaasCard();
            $this->webhook = new AsaasWebhook($this->client, $webhookToken);
        }
        ...
        public function createCheckout(int $userId, int $planId, string $billingCycle, array $paymentData = []): PaymentCheckoutResult
        {
            ...
            $customerId = $this->customer->findOrCreateCustomer($user);
            ...
            $paymentStatus = $firstPayment
                ? $this->webhook->paymentStatusFromAsaasStatus((string) ($firstPayment['status'] ?? ''))
                : 'pending';
            ...
            if ($paymentMethod === 'credit_card' && $paymentStatus === 'failed') {
                ...
                return PaymentCheckoutResult::error($this->client->errorMessage($firstPayment) ?: 'Cartao recusado pelo Asaas. Confira os dados ou tente outro cartao.');
            }

            $pixQrCode = $providerPaymentId !== '' ? $this->pix->pixQrCodeForPayment($providerPaymentId) : [];
            $redirectUrl = $this->webhook->checkoutUrlFromResponse($paymentSource);
            if ($redirectUrl === '') {
            ...
            return PaymentCheckoutResult::success($redirectUrl, $activatedSubscription ? (int) $activatedSubscription['id'] : null, [
                ...
                'asaas_payment_url' => $this->webhook->checkoutUrlFromResponse($paymentSource),
                'external_reference' => $externalReference,
        ...
        {
            $this->webhook->validateWebhookToken($headers);
            ...
            $newStatus = $this->webhook->subscriptionStatusFromEvent($event) ?? $this->webhook->subscriptionStatusFromPaymentStatus($paymentStatus);
        ...
        public function syncCheckoutPayment(int $userId, string $providerSubscriptionId): array
        {
            ...
            $paymentStatus = $this->webhook->paymentStatusFromAsaasStatus((string) ($payment['status'] ?? ''));
        ...
        public function confirmSandboxPayment(string $providerPaymentId): array
        {
            ...
            if (!str_contains($this->client->getApiUrl(), 'api-sandbox.asaas.com')) {
                return [
                    'ok' => true,
                    'provider' => $this->name(),
                    'sandbox_confirmed' => false,
            ...
            $payment = $this->client->request('POST', '/sandbox/payment/' . rawurlencode($providerPaymentId) . '/confirm');
        ...
        {
            try {
                $payments = $this->client->request('GET', '/payments', [
        ...
        private function latestPaymentForSubscription(string $providerSubscriptionId): array
        {
            ...
            foreach ($items as $payment) {
                if (is_array($payment) && $this->webhook->paymentStatusFromAsaasStatus((string) ($payment['status'] ?? '')) === 'paid') {
                    return $payment;
                }
            }
...
namespace {
    if (!class_exists('AsaasPaymentProvider')) {
        class_alias('App\Services\Payments\AsaasPaymentProvider', 'AsaasPaymentProvider');
    }
}
