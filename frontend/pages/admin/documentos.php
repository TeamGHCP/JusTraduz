...
$documents = fetch_all(
    $pdo,
    'SELECT d.id, d.nome_arquivo, d.tipo_arquivo, d.created_at, u.nome AS usuario, u.email, ar.id AS analysis_id
     FROM documents d
...
<body>
  <div class="app-shell admin-shell">
    ...
    <main class="app-main">
      <?php render_topbar('Documentos', 'Audite envios e acompanhe análises por IA.', current_user_name()); ?>

      <section class="grid grid-3">
        <?= stat_card('Total de documentos', count($documents), 'folder') ?>
        <?= stat_card('Analisados por IA', count(array_filter($documents, fn ($doc) => !empty($doc['analysis_id']))), 'chart') ?>
        <?= stat_card('Pendentes', count(array_filter($documents, fn ($doc) => empty($doc['analysis_id']))), 'help') ?>
      </section>
      ...
      <section class="dash-section">
        ...
        <?php else: ?>
          <div class="table-wrap">
            <table class="table">
              <thead><tr><th>Documento</th><th>Usuário</th><th>Tipo</th><th>Análise</th><th>Enviado em</th><th>Ação</th></tr></thead>
              <tbody>
