...
if ($downloadMode) {
    ...
    simple_pdf_download('fatura-' . $documentNumber . '.pdf', [
        'title' => 'Fatura',
        'number' => $documentNumber,
        'meta' => [
            'Emissão' => billing_date($date),
            'Status' => $statusLabel,
        ],
        'left_title' => 'Emitido por',
        'left_lines' => ['JusTraduz', 'Plataforma jurídica digital', 'Brasil'],
        'right_title' => 'Cliente',
        'right_lines' => $clientLines,
        'summary' => billing_money($amountCents) . ' ' . ($statusLabel === 'Pago' ? 'pago' : 'a vencer') . ' em ' . billing_date($date),
        'headers' => ['Descrição', 'Método', 'Valor'],
        'columns' => [300, 105, 95],
        'rows' => [[
        ...
        'totals' => [
            'Subtotal' => billing_money($amountCents),
            'Total' => billing_money($amountCents),
            'Valor devido' => $statusLabel === 'Pago' ? 'R$ 0,00' : billing_money($amountCents),
        ],
        'footer' => array_filter([
            $providerPaymentId !== '' ? 'Pagamento Asaas: ' . $providerPaymentId : '',
